...
@Composable fun ExpensePane(a:ComponentActivity){
 ...
 val canManage=SecurityStorage.canManage(a)
 Column{
  Text("প্রতিষ্ঠানের দৈনন্দিন/মাসিক খরচ")
  if(!canManage) Text("শুধু Admin খরচ যোগ করতে পারবেন।",color=MaterialTheme.colorScheme.error)
  OutlinedTextField(date,{date=it},label={Text("তারিখ (YYYY-MM-DD)")},modifier=Modifier.fillMaxWidth())
  OutlinedTextField(cat,{cat=it},label={Text("খরচের ধরন")},modifier=Modifier.fillMaxWidth())
  OutlinedTextField(amt,{amt=it},label={Text("টাকা")},modifier=Modifier.fillMaxWidth())
  OutlinedTextField(note,{note=it},label={Text("বিবরণ")},modifier=Modifier.fillMaxWidth())
  Button({
   if(!canManage){msg="শুধু Admin খরচ যোগ করতে পারবেন।"} else { val x=amt.toDoubleOrNull(); when{
    date.length!=10->msg="সঠিক তারিখ দিন: YYYY-MM-DD"
    cat.isBlank()->msg="খরচের ধরন দিন।"
    x==null||x<=0->msg="সঠিক টাকার পরিমাণ দিন।"
    else->{ExpenseStorage.addExpense(a,Expense(System.currentTimeMillis(),date,cat.trim(),x,note.trim()));ActivityLogStorage.add(a,"প্রতিষ্ঠানের খরচ যোগ",cat.trim()+" — ৳ "+String.format(Locale.US,"%.2f",x));list=ExpenseStorage.getExpenses(a);amt="";note="";msg="খরচ সংরক্ষণ হয়েছে।"}
   }}
  },Modifier.fillMaxWidth(),enabled=canManage){Text("খরচ যোগ করুন")}
  if(msg.isNotBlank())Text(msg)
  Text("মোট: ৳ %.2f".format(Locale.US,list.sumOf{it.amount}))
...
@Composable fun DamagePane(a:ComponentActivity){
 ...
 Column{Text("ড্যামেজ/নষ্ট/ভাঙা পণ্য");if(!canManage)Text("শুধু Admin ড্যামেজ সংরক্ষণ করতে পারবেন।",color=MaterialTheme.colorScheme.error);OutlinedTextField(date,{date=it},label={Text("তারিখ")},modifier=Modifier.fillMaxWidth());OutlinedTextField(code,{code=it},label={Text("পণ্যের কোড")},modifier=Modifier.fillMaxWidth());OutlinedTextField(qty,{qty=it},label={Text("পরিমাণ")},modifier=Modifier.fillMaxWidth());OutlinedTextField(reason,{reason=it},label={Text("কারণ")},modifier=Modifier.fillMaxWidth());Button({if(!canManage){msg="শুধু Admin ড্যামেজ সংরক্ষণ করতে পারবেন."}else{val p=products.firstOrNull{it.code.equals(code.trim(),true)};val q=qty.toIntOrNull();when{p==null->msg="পণ্যের কোড মেলেনি।";q==null||q<=0->msg="সঠিক পরিমাণ দিন।";q>p.stockQuantity->msg="স্টকে এত পরিমাণ নেই।";else->{val stockUpdated = ProductStorage.updateStock(a,p.code,p.stockQuantity-q);if(!stockUpdated){msg="ড্যামেজের স্টক আপডেট করা যায়নি।";return@Button};ExpenseStorage.addDamage(a,DamageRecord(System.currentTimeMillis(),date,p.code,p.name,q,p.purchasePrice,reason));ActivityLogStorage.add(a,"ড্যামেজ যোগ",p.name+" × "+q);list=ExpenseStorage.getDamages(a);msg="ড্যামেজ যোগ হয়েছে এবং স্টক কমেছে।";code="";qty="";reason=""}}}},Modifier.fillMaxWidth(),enabled=canManage){Text("ড্যামেজ সংরক্ষণ")};if(msg.isNotBlank())Text(msg);Text("মোট ক্ষতি: ৳ %.2f".format(Locale.US,list.sumOf{it.totalLoss}));LazyColumn{items(list.take(50)){Text(it.date+" — "+it.productName+" × "+it.quantity+": ৳ "+String.format(Locale.US,"%.2f",it.totalLoss),Modifier.padding(5.dp))}}}}
